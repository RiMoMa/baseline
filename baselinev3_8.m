...
if conf.tinyProblem
  conf.prefix = [NoCarpeta,'_Baseline_5vabril'] ;
  conf.numClasses = 3 ;
  conf.numSpatialX = 1;
  conf.numSpatialY = 1;
  conf.numWords = 1600 ;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% algoritmo buscada K-fold
%%NoCarpeta

%%%aux configurar para que lea el nombre del for

indices_test=[];
for fd=1:length(dataset)
aux=cell2mat(dataset(1,fd));
aux=aux(1:3);

testigo = strcmp(aux,NoCarpeta);
 if testigo
     indices_test=[indices_test,fd];
...
for n=1:length(indices_test)
    
    switch cell2mat(dataset(2,indices_test(n)))
        case 1
            index1=[index1,indices_test(n)];
        case 2
            index2=[index2,indices_test(n)];
        case 3
            index3=[index3,indices_test(n)];
    end
    
    
end

       % indices_train = setdiff(1:length(y_labels),selTrain_svm);       
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        index1_aux=vl_colsubset(1:length(aux1),conf.numTrain+conf.numTest);
        index2_aux=vl_colsubset(length(aux1)+1:length(aux1)+length(aux2),conf.numTrain+conf.numTest);
        index3_aux=vl_colsubset(length(aux1)+length(aux2)+1:length(aux1)+length(aux2)+length(aux3),conf.numTrain+conf.numTest);
        aux=randperm(conf.numTrain+conf.numTest);
        index1_aux=index1_aux(aux);
        index2_aux=index2_aux(aux);
        index3_aux=index3_aux(aux);
        
        
        index1a=setdiff(index1_aux,index1);
        
        index2a=setdiff(index2_aux,index2);
        
        index3a=setdiff(index3_aux,index3);
        
        
        
        
        indices_imagenes=[index1_aux,index2_aux,index3_aux];
        selTrain=cat(2,index1_aux(1:conf.numTrain),index2_aux(1:conf.numTrain),index3_aux(1:conf.numTrain));
        selTest=cat(2,index1_aux(conf.numTrain+1:end),index2_aux(conf.numTrain+1:end),index3_aux(conf.numTrain+1:end));
        selTrain_svm=[1:length(index1_aux)-conf.numTest];
        selTrain=cat(2,index1_aux(1:conf.numTrain),index2_aux(1:conf.numTrain),index3_aux(1:conf.numTrain));
        selTest=cat(2,index1_aux(conf.numTrain+1:end),index2_aux(conf.numTrain+1:end),index3_aux(conf.numTrain+1:end));
        
        y_labels=[1*ones(1,conf.numTrain+conf.numTest+index1),2*ones(1,conf.numTrain+conf.numTest+index2),3*ones(1,conf.numTrain+conf.numTest+index3)]  ; 
...
if ~exist(conf.histPath) || conf.clobber
  ...
  for ii=1:(length(selTrain)+length(selTest))
   ...
   im={im_aux1,im_aux2,im_aux3,imselTrain_svm_aux4};
